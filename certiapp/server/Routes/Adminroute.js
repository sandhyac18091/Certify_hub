...
Adminroute.post('/signup',async(req,res)=>{
    try{
        ...
        const existinguser=await certidapp.findOne({email:Email})
    if(existinguser){
...
Adminroute.post('/login',async(req,res)=>{
   try{
    ...
    const result=await certidapp.findOne({email:Email})
    console.log(result);
    ...
    if(!result){
        ...
        if(compare){
            const token=jwt.sign({Email:result.email,Userrole:result.userrole},seceretkey,{expiresIn:'1hr'})
            console.log(token);
...
Adminroute.post('/issuecertificate',authenticate,async(req,res)=>{
   try{
